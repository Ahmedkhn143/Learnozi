...
export async function updateUserPreferences(userId, newPreferences) {
  ...
  if (isValidUUID(userId)) {
    try {
      ...
      console.warn('Supabase updateUserPreferences fallback:', err.message);
    }
  }
  ...
  if (idx !== -1) {
    users[idx].preferences = updatedPrefs;
    writeLocalUsers(users);
  }
...
export async function updateUserProfile(userId, { name, academicProfile }) {
  if (!userId) return null;
  const updateData = { updated_at: new Date().toISOString() };
  if (name) updateData.name = name;
  if (academicProfile) updateData.academic_profile = academicProfile;
  ...
  if (isValidUUID(userId)) {
    try {
      ...
      console.warn('Supabase updateUserProfile fallback:', err.message);
    }
  }
  ...
  if (idx !== -1) {
    ...
    if (academicProfile) {
      users[idx].academic_profile = academicProfile;
      users[idx].academicProfile = academicProfile;
    }
    writeLocalUsers(users);
  }
