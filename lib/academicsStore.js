...
function ensureAcademicsDataDir() {
  ...
  if (!fs.existsSync(ACADEMICS_FILE)) {
    ...
    fs.writeFileSync(ACADEMICS_FILE, JSON.stringify(defaultData, null, 2), 'utf-8');
  }
}
...
export async function getSemesters(userId) {
  ...
  if (isValidUUID(userId)) {
    try {
      ...
      if (!semErr && Array.isArray(semesters) && semesters.length > 0) {
      ...
      console.warn('Supabase getSemesters fallback:', err.message);
    }
  }

  // 2. Fallback to local store
  const allData = readLocalAcademics();
  let userSemesters = allData.filter((s) => s.userId === userId || s.userId === 'demo_user_123');
  if (userSemesters.length === 0) {
    userSemesters = allData.filter((s) => s.userId === 'demo_user_123');
  }
